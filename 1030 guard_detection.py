...
class BoxingDefenseDetector:
    ...
    def __init__(self, cooldown=1.0, history_len=20):
        ...
        self.neck_y_velocity = {}  # 每個玩家的頸部垂直速度歷史
        self.last_neck_y = {}  # 上一幀的頸部高度
        self.velocity_history_len = 5
    ...
    @staticmethod
    def _get_keypoint(keypoints, person_id: int, index: int, confidence_threshold=0.3):
        """安全獲取關鍵點 - 適配雙人檢測"""
        try:
    ...
    def _update_stance_baseline(self, person_id: int, keypoints):
        ...
        # 初始化速度追蹤
        if person_id not in self.neck_y_velocity:
            self.neck_y_velocity[person_id] = deque(maxlen=self.velocity_history_len)
            self.last_neck_y[person_id] = None

        # 計算速度（像素/幀）
        if neck is not None and self.last_neck_y[person_id] is not None:
            velocity = neck[1] - self.last_neck_y[person_id]
            self.neck_y_velocity[person_id].append(velocity)

        if neck is not None:
            self.last_neck_y[person_id] = neck[1]
    ...
    def _detect_slip(self, person_id: int, keypoints_dict, shoulder_width):
        ...
        if significant_drop or significant_lateral:
            ...
            magnitude_score = max(drop_score, lateral_score)

            # 2. 速度分數（新增）
            velocity_score = 0.5  # 預設值
            if len(self.neck_y_velocity[person_id]) >= 3:
                recent_velocities = list(self.neck_y_velocity[person_id])[-3:]
                avg_velocity = np.mean(recent_velocities)

                # 正規化速度（以肩寬為單位）
                normalized_velocity = abs(avg_velocity) / shoulder_width
            ...
            if len(self.neck_y_velocity[person_id]) >= 4:
                ...
                # 正規化加速度
                normalized_acc = avg_acceleration / shoulder_width
                acceleration_score = min(normalized_acc / 0.1, 1.0)

            # 4. 方向一致性分數（新增）
            consistency_score = 0.5
            if len(self.neck_y_velocity[person_id]) >= 3:
                velocities = list(self.neck_y_velocity[person_id])[-3:]
                # 檢查速度方向是否一致（都是下降或都是側移）
                if significant_drop:
                    # 下降潛避：速度應都為正（y增加）
                    consistent_direction = all(v > 0 for v in velocities)
                else:
                    # 側移潛避：速度方向可以混合，但絕對值要大
                    consistent_direction = all(abs(v) > shoulder_width * 0.05 for v in velocities)

                consistency_score = 1.0 if consistent_direction else 0.6

            # 綜合置信度計算
            confidence = (
                                 magnitude_score * 0.35 +  # 幅度權重 35%
                                 velocity_score * 0.3 +  # 速度權重 30%
                                 acceleration_score * 0.2 +  # 加速度權重 20%
                                 consistency_score * 0.15  # 一致性權重 15%
                         ) * 0.9  # 整體調整係數
    ...
    def _detect_shell_guard(self, keypoints_dict, left_angle, right_angle, shoulder_width):
        ...
        # 1. 角度檢查：雙臂必須緊縮
        angles_in_range = (config.angle_range[0] <= left_angle <= config.angle_range[1] + 20 and
                           config.angle_range[0] <= right_angle <= config.angle_range[1] + 20)
        ...
        r_wrist = keypoints_dict[RIGHT_WRIST]
        l_shoulder = keypoints_dict[LEFT_SHOULDER]
        r_shoulder = keypoints_dict[RIGHT_SHOULDER]
        neck = keypoints_dict[NECK]

        if any(p is None for p in [l_elbow, r_elbow, l_wrist, r_wrist, l_shoulder, r_shoulder]):
            return None

        # 計算身體中線
        body_center_x = (l_shoulder[0] + r_shoulder[0]) / 2

        # 檢測雙手交叉：左手腕在中線右側 且 右手腕在中線左側
        left_wrist_crosses = l_wrist[0] > body_center_x
        right_wrist_crosses = r_wrist[0] < body_center_x
        is_crossed = left_wrist_crosses and right_wrist_crosses

        # 計算交叉程度（正規化）
        cross_distance = 0
        if is_crossed:
            left_cross_dist = (l_wrist[0] - body_center_x) / shoulder_width
            right_cross_dist = (body_center_x - r_wrist[0]) / shoulder_width
            cross_distance = (left_cross_dist + right_cross_dist) / 2
        ...
        elbows_close = right_elbow_dist_norm < 0.4 and left_elbow_dist_norm < 0.4

        # 手腕高度檢測（應在胸部到頭部之間）
        chest_level = (l_shoulder[1] + r_shoulder[1]) / 2
        wrists_at_guard_height = (
                chest_level - shoulder_width * 0.8 < l_wrist[1] < neck[1] + 30 and
                chest_level - shoulder_width * 0.8 < r_wrist[1] < neck[1] + 30
        )

        # 判定：交叉防禦 或 緊貼防禦
        if (is_crossed and cross_distance > 0.15 and wrists_at_guard_height) or \
                (elbows_close and wrists_at_guard_height):

            # 計算置信度
            optimal_angle = (config.angle_range[0] + config.angle_range[1]) / 2
            angle_score = 1.0 - (abs(left_angle - optimal_angle) + abs(right_angle - optimal_angle)) / 100.0
            angle_score = max(0, min(angle_score, 1.0))

            # 位置分數
            if is_crossed:
                # 交叉防禦：交叉程度越大分數越高
                pos_score = min(cross_distance / 0.4, 1.0)
            else:
                # 緊貼防禦：手肘越貼近分數越高
                pos_score = 1.0 - (right_elbow_dist_norm + left_elbow_dist_norm) / 0.8
            pos_score = max(0, min(pos_score, 1.0))

            # 高度分數
            height_score = 1.0 if wrists_at_guard_height else 0.5

            confidence = (angle_score * 0.4 + pos_score * 0.4 + height_score * 0.2) * 0.95

            if confidence > 0.45:  # 降低門檻以提高辨識率
                defense_name = "貝殼防禦(交叉)" if is_crossed else "貝殼防禦(緊貼)"
                return {
                    "type": DefenseType.SHELL_GUARD.value,
                    "name": defense_name,
                    "confidence": round(confidence,3),
                    "effectiveness": config.effectiveness * (1.05 if is_crossed else 1.0),
                    "hands": "both",
    ...
    @staticmethod
    def _detect_single_hand_block(keypoints_dict, hand, angle, config, shoulder_width):
        """檢測單手格擋"""
...
class BoxingDefenseVisualizer:
    ...
    def draw_defense_frame(self, frame, keypoints, defense_data: DefenseFrameData):
        ...
        # 繪製系統狀態
        self._draw_system_status(result_frame, defense_data, width)
    ...
    @staticmethod
    def _draw_system_status(frame, defense_data,height):
        """繪製防禦系統狀態信息"""
        # 背景框
