...
經本校12/29專題展後，針對Python後處理的部分，往後需調整的方向為對於這種連續幀的影像行為，需用狀態與事件的想法下去做改良，也就是說，原本採用的多維度特徵辨識手法為一種撇除深度學習大數據庫的建置餵資料方式，而一開始所參照實驗室處理的這種方式為一種折衷的辦法，因此原本的設計為連續幀的影像，可能某一幀的ground_truth_action(手動輸入，為影片當下所預設做的動作)與detected_action(OpenPose當下幀辨識的成果)一致且置信度高即為該動作辨識成功，現在需要設計一個上鉤拳，可能有State 0: IDLE (預備狀態)
  - 特徵: 雙手在身體兩側或腰部附近
  - 速度: 低速或靜止
  - 轉移條件: 偵測到「啟動特徵」

State 1: PREPARING (蓄力階段)
  - 特徵: 手臂開始後拉/彎曲/下沉
  - 速度: 中低速,有方向性
  - 持續時間: 3-8幀(可調)
  - 轉移條件: 累積動量達閾值

State 2: EXECUTING (爆發階段)
  - 特徵: 手腕高速移動 + 角度快速變化
  - 速度: 峰值速度
  - 持續時間: 2-5幀(拳擊動作很快)
  - 轉移條件: 速度開始下降 OR 達到終點姿態

State 3: RECOVERY (收回階段)
  - 特徵: 手臂回到防禦位置
  - 速度: 中速衰減
  - 持續時間: 5-10幀
  - 轉移條件: 回到IDLE特徵範圍

State 4: HOLDING (定格狀態)
  - 特徵: 維持某個姿勢不動
  - 速度: 接近0
  - 用途: 區分「防禦格擋」vs「攻擊後停頓」

這樣子類似狀態機的設計，且由於作者本身的設計動作若使用這樣子的判斷也很容易造成誤判(在2D影像的環境下)，因此未來若要持續擴展，得先往這個方向進行設計。
